...
def coeffs_to_tensor(coeffs, device):  
    ...
    for cA,cH,cV,cD in coeffs:
        ...
        cV = torch.from_numpy(cV).to(device)
        cD = torch.from_numpy(cD).to(device)
        t = torch.stack([cA,cH,cV,cD],dim=0)
        tensors.append(t)
...
def inverse_wavelet_transform(coeffs, original_size, device):  
    ...
    for cf in coeffs:
        ...
        recs.append(rec)
    return torch.stack(recs,dim=0).unsqueeze(1)
...
class WTDDPMSampler_cond(nn.Module):
    ...
    def forward(self,cbct):
        with torch.no_grad():
            ...
            ct_coeffs=tensor_to_coeffs(d_t)
